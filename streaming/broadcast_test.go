...
func TestPostUpdateEventsUseReliableClusterSend(t *testing.T) {
	...
	t.Run("text, control, reasoning and annotation events are reliable", func(t *testing.T) {
		client := newClient()
		...
		assertAllPostUpdatesReliable(t, client.events)
	})

	t.Run("tool call events are reliable for requester and channel broadcasts", func(t *testing.T) {
		client := newClient()
		service := newService(client)
