...
func (s *webSearchService) resolve(llmContext *llm.Context, argsGetter llm.ToolArgumentGetter) (string, error) {
	...
	if searchResp.Answer != "" {
		if unsafe, reason := isSummaryUnsafe(searchResp.Answer); unsafe {
			s.logWarn("provider summary rejected", "reason", reason)
			builder.WriteString("Provider summary was discarded due to safety checks. Use the sources below.\n\n")
			searchResp.Answer = ""
		} else {
			builder.WriteString("\nSummary:\n")
...
func (s *webSearchService) resolveSource(bot *bots.Bot, llmContext *llm.Context, argsGetter llm.ToolArgumentGetter) (string, error) {
	...
	if unsafe, reason := isSummaryUnsafe(summary); unsafe {
		s.logWarn("summarized content rejected", "reason", reason)
		return s.wrapSourceContentWithContext(summary, matchedResult, llmContext), nil
	}
...
func isSummaryUnsafe(summary string) (bool, string) {
	...
	indicators := []string{
		"system:", "assistant:", "user:", "developer:", "role:",
		"function_call", "tool_calls", "\"tool_calls\":", "\"function\":", "\"arguments\":", "\"name\":", "\"type\":\"function\"",
		"ignore previous", "ignore all previous", "disregard", "forget previous", "reset instructions",
	...
	for _, phrase := range indicators {
		if strings.Contains(lower, phrase) {
			return true, "contains directive: " + phrase
		}
	}
