...
// Package profile defines the load-test mock LLM profile: its defaults,
// operator-JSON overlay parsing, and validation. It is a leaf package so that
// both llm (config validation) and loadtest (mock execution) can use it.
package profile
...
func (t ToolArgumentProfile) clone() ToolArgumentProfile {
	t.PostLimits = slices.Clone(t.PostLimits)
	t.SearchQueries = slices.Clone(t.SearchQueries)
	t.SearchLimits = slices.Clone(t.SearchLimits)
	t.MessageLengths = slices.Clone(t.MessageLengths)
	t.Usernames = slices.Clone(t.Usernames)
	t.ChannelIDs = slices.Clone(t.ChannelIDs)
	t.ChannelNames = slices.Clone(t.ChannelNames)
	t.TeamIDs = slices.Clone(t.TeamIDs)
	t.TeamNames = slices.Clone(t.TeamNames)
	t.PostIDs = slices.Clone(t.PostIDs)
	return t
}
...
// Clone returns a copy of the profile whose maps and slices are independent of
// the receiver's, so callers can hold a profile without seeing later mutations.
func (p MockProfile) Clone() MockProfile {
	p.LatencyProfiles = maps.Clone(p.LatencyProfiles)
	p.ProfileWeights = maps.Clone(p.ProfileWeights)
	p.ToolWeights = maps.Clone(p.ToolWeights)
	if p.ToolArgumentProfiles != nil {
		taps := make(map[string]ToolArgumentProfile, len(p.ToolArgumentProfiles))
		for k, v := range p.ToolArgumentProfiles {
			taps[k] = v.clone()
		}
		p.ToolArgumentProfiles = taps
	}
	p.FinalResponseTemplates = slices.Clone(p.FinalResponseTemplates)
	return p
}
...
// Parse merges operator JSON on top of the default profile. Nil, empty, or whitespace-only raw returns the default.
func Parse(raw json.RawMessage) (MockProfile, error) {
...
func validateWeightMap(m map[string]float64, name string) error {
	if len(m) == 0 {
...
func (p MockProfile) Validate() error {
	...
	if err := validateWeightMap(p.ProfileWeights, "profile_weights"); err != nil {
		return err
	}
	...
	if err := validateWeightMap(p.ToolWeights, "tool_weights"); err != nil {
		return err
	}
