...
func (r *React) Resolve(message string, context *llm.Context) (string, error) {
	...
	emojiName, err := r.llm.ChatCompletionNoStream(stdcontext.Background(), completionRequest, llm.WithMaxGeneratedTokens(500), llm.WithReasoningDisabled(), llm.WithToolsDisabled())
	if err != nil {
