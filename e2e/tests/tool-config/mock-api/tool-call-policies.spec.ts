...
const embeddedReadPostLabel = 'Mattermost Read Post';
const embeddedReadChannelLabel = 'Mattermost Read Channel';
const embeddedGetChannelInfoLabel = 'Mattermost Get Channel Info';
...
test.describe('Tool Call Policies (Mocked LLM)', () => {
    ...
    test('approval continuation creates a second post that does not duplicate the first post tools or show the empty-result fallback', async ({ page }) => {
        ...
        await expect(postB.getByText(embeddedGetChannelInfoLabel, {exact: true})).not.toBeVisible();
    });
