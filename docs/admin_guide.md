...
- **Semantic search is unavailable** from the moment the reindex starts until the final index build completes. Search requests return an error (HTTP 503 on the API) during this window. Live posts continue to be indexed while the bulk load runs; while the final index build itself runs — which can be hours on a large database — live post indexing (new posts and edits), post deletions, and retention cleanup are paused instead of blocking on the build. Everything paused is repaired after the build: new posts by the catch-up pass, edits and deletions by the repair pass, and retention by its next scheduled run.
- **After the build, edited posts are repaired.** Once the index is rebuilt, search becomes available again and the job enters a short `repairing` phase: posts edited while live indexing was paused are re-embedded, and new posts are swept up by the catch-up pass. If the job is canceled or fails after the build but before the repair completes, the pending repair is recorded durably — the health check shows the `repairing` state, and resuming the job (or running a reindex) completes the repair. Search works during this phase; a few recently edited posts may return slightly stale results until the repair finishes.
- **Tune the index build on the database side** — the plugin intentionally does not override server settings. The build only stays fast while the whole HNSW graph fits in `maintenance_work_mem` — budget roughly `rows × (4 × dimensions + 300)` bytes, where the constant covers per-element graph overhead that dominates at low dimensions (measured: ~1.7 KB/element at 256 dimensions, ~34 GB for 20M posts). If the graph outgrows that budget, PostgreSQL switches to on-disk insertion for the remaining rows and build speed drops by more than an order of magnitude (~40x measured), so size the setting for the full graph — temporarily on a bigger instance if needed. pgvector 0.6+ can also parallelize the build with `max_parallel_maintenance_workers`.
- **Crash recovery**: the index lifecycle state is durable. If the plugin or server restarts mid-reindex, the health check (**Check Index Health**) reports the vector index state, and search stays disabled until the index is rebuilt. Resume the reindex job (or start a new full reindex) to rebuild the index; the plugin never rebuilds it implicitly during activation because the build can take hours. Any full or resumed reindex rebuilds a dropped index (and completes a pending repair), even if the strategy setting has since been changed back to `maintain`.
