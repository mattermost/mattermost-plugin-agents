...
| Setting | Description |
|---------|-------------|
| **Enable Web Search** | Available for Anthropic, OpenAI, Google Gemini, and Google Vertex AI. For OpenAI Compatible and Azure, this setting is available when **Use Responses API** is enabled on the Service. Gemini and Vertex map this to Google Search grounding via the provider's Responses API. Allows the Agent to leverage the provider's native web search tool to respond with recent information. For Anthropic, the plugin pins the web search tool's `allowed_callers` to `direct`, opting out of Anthropic's [dynamic filtering](https://platform.claude.com/docs/en/agents-and-tools/tool-use/web-search-tool#dynamic-filtering) so newer Claude models (4.6 and later) don't auto-provision Anthropic's code-execution sandbox. |
