...
Records identify the actor (user ID, session, IP), the event, the request path, the outcome (including failures and permission denials), the OpenTelemetry `trace_id` for pivoting into traces, and identifiers of affected objects using the same attribute names as the table above. Records never contain prompt content, conversation or channel content, tool arguments or results, or configuration values — a configuration save records only which top-level sections changed.
