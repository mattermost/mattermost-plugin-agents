...
| **Max tool turns** | Maximum LLM rounds in the tool loop per response (includes a final synthesis round without calling tools). Default **30**, range **1–250**. Lower for smaller models that loop on tool calls; raise for agents that chain many MCP tools per turn. |
