...
When enabled, a user who joins a public or private channel is shown a private, ephemeral "catch-up" summary of the channel's recent activity, generated by an agent from recent messages. The summary is visible only to the joining user and is not stored. Direct messages, group messages, and archived channels are never summarized.

Each qualifying join runs one LLM completion, so enable this deliberately on busy servers.

To configure:

1. Navigate to **System Console > Plugins > Agents > Channel Join Summary**
2. Set **Enable Channel Join Summary** to **True**
3. Optionally set:

   - **Summary Agent** — the agent whose model generates the summary. Leave as **First configured agent** to use the default.
   - **Lookback (days)** — how far back the recent-activity window reaches (default 7).
   - **Minimum Messages** — skip the summary when the window has fewer than this many eligible messages, to avoid interrupting joins to quiet channels (default 3).
