...
2. On the agent's **MCPs** tab (Agents page), turn on **Use service accounts for authentication**. Only system administrators can turn this setting on or modify the agent while it is enabled; anyone who can manage the agent can still view its configuration, turn the setting off, or delete the agent.
