...
- **Display name**, **avatar**, **model**, **max tool turns**, **custom instructions**, **vision**, **reasoning**, and **structured output** can be changed by anyone who can manage the agent at any time — including while **Use service accounts for authentication** is enabled. **Service** (AI provider), **tools**, **native tools**, **channel access**, **user access**, **agent admins**, and **MCP tool grants** can also be changed freely when service account authentication is off; while it is on, only system administrators can change those sensitive settings. Anyone who can manage the agent may still turn service account authentication off or delete the agent. Enabling service account authentication itself is system-admin-only.
