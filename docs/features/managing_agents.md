...
| **Max tool turns** | Maximum LLM rounds in the tool loop per response (includes a final synthesis round without calling tools). Defaults to **30** (allowed range **1–250**). Lower this for smaller models that tend to loop on tool calls; raise it for agents that chain many tools per turn (for example dynamic MCP discovery: search → load → execute). Clearing the field saves the default. |
