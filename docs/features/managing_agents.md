...
- While service account authentication is on, an **Advanced configuration** section offers two experimental, system-admin-only settings: **Skip tool call approvals** and **Use the agent's bot account permissions**. See [Experimental service account options](../admin_guide.md#experimental-service-account-options).
