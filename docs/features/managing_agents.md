...
- **Display name**, **avatar**, **service**, **model**, **max tool turns**, **custom instructions**, **vision**, **tools**, **native tools**, **reasoning**, **structured output**, **channel access**, **user access**, **agent admins**, and the **MCP tool grants** can all be changed at any time — except while **Use service accounts for authentication** is enabled, when only system administrators can save changes (anyone who can manage the agent may still view its configuration, turn that setting off, or delete the agent).
