...
| **Max tool turns** | Optional. Maximum consecutive tool-call/execute rounds the agent runs per turn before stopping. Defaults to **30** (valid range **1–250**). Lower this for smaller models that tend to loop on tool calls; raise it for agents that chain many tools (for example dynamic MCP discovery: search → load → execute). Clearing the field saves the default. |
