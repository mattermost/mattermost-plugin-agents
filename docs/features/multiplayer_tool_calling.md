...
- `auto_run_everywhere` is for read-only tools whose results are unambiguously safe to share with everyone in any channel. The vetted-provider seeding flow (PR #520) installs the embedded Mattermost built-in tools (`read_post`, `read_channel`, `get_channel_info`, `get_channel_members`, `get_team_info`, `get_team_members`, `search_posts`, `search_users`, `get_user_channels`, `read_file`) at `auto_run_in_dm` by default. `read_file` is still read-only, but it stays in the DM-only bucket because file contents may be acceptable to reveal to the initiator privately while still needing explicit consent before they are exposed in a channel. Admins must explicitly promote any of these to `auto_run_everywhere` if they want them to skip approval in channels — and given the privacy implications described in section 6, that decision should be deliberate.
