...
File creation supports text-based formats — the file name's extension determines the file type (such as `.md`, `.csv`, `.py`, or `.html`) — and an agent can attach up to 10 files to a single reply. In direct messages, file creation is always available. In channels, it follows the same admin setting as other channel tool calling (see [Use tools](#use-tools)).
