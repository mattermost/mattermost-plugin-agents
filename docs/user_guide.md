...
When an Agent needs you to pick from specific options before continuing, it may show a question card on the bot post. Select one or more options (numbered for single-select, checkboxes for multi-select), optionally type a custom answer using **Something else…**, then click **Accept**. Click **Skip** to decline; the Agent continues without your answer. Only the person who started the Agent interaction can answer or skip. If the same response also includes auto-approved tools that were waiting on your answer, those tools run once you submit or skip and appear as **Auto-approved** rows.
