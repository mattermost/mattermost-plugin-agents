...
When you attach files in the Agents pane, direct messages, or channel @mentions, Agents can use them as conversation context. Images are included only when vision is enabled for the bot. Other readable files at or below 8 KiB contribute text directly. Larger files are included as metadata, including the File ID, and Agents can read their text on demand with the `read_file` tool when text is available. Files without extractable text can't be read as text. In channels, reading a larger file may require approval before the content is fetched.
