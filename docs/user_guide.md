...
If the immediately preceding post in a channel thread was from an Agent and you reply without `@mention`ing an Agent again, Mattermost shows an inline reminder with a link to loop that same Agent back in. Selecting the link processes your existing reply through the normal channel-mention flow for that Agent, so the same channel tool-calling and approval rules apply. This reminder is available only in channel threads, not top-level posts, direct messages, or group messages, and only when you are allowed to use that Agent in the channel.
