...
When a tool runs automatically or after you approve it, Mattermost continues the response in the same bot post instead of creating a separate follow-up post. That post can show multiple rounds of the response, so earlier tool cards and text stay visible while the newest answer appears in the last round.
...
If you use **Regenerate** on a response that already includes tool rounds, Mattermost replaces the generated rounds in that same post and streams the replacement as a fresh answer instead of appending another follow-up round to the old result.
