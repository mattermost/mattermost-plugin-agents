...
export interface PostActivity {
     ...
     * including the last one that has tool calls.
     */
    activityRounds: Round[];

    /**
     * The rounds after the last tool call. Their text is the answer and
     * renders as the normal post message.
     */
    answerRounds: Round[];
...
export function deriveActivity(rounds: Round[]): PostActivity {
    ...
    const activityRounds = rounds.slice(0, lastToolRoundIdx + 1);
