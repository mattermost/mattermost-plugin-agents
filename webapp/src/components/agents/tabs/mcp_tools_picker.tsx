...
export const MCPServerToolWildcard = '*';
...
const MCPToolsPicker = (props: Props) => {
    const {enabledTools, autoEnableNewMCPTools, onChange, showAutoEnable = true} = props;
    const intl = useIntl();
    ...
    if (servers.length === 0) {
        return (
            <EmptyContainer>
                {props.emptyMessage ?? <FormattedMessage defaultMessage='No MCP servers are configured. Ask your system administrator to configure MCP servers in the system console.'/>}
            </EmptyContainer>
        );
    }
    ...
    return (
        <Container>
            ...
            {autoEnableNewMCPTools && (
                <AutoEnableBanner>
                    <FormattedMessage defaultMessage='Every MCP tool is enabled. Disable automatic enablement above to pick specific tools.'/>
                </AutoEnableBanner>
            )}
                            ...
                            {isExpanded && (
                                ...
                                >
                                    {adminEnabledTools.length === 0 && wildcardOn && (
                                        <EmptyToolsNotice>
                                            <FormattedMessage defaultMessage='This server has no tools available right now. When a user authenticates, every tool this server exposes will be enabled.'/>
                                        </EmptyToolsNotice>
                                    )}
                                    {adminEnabledTools.length === 0 && !wildcardOn && canConnect && (
                                        <EmptyToolsNotice>
                                            <FormattedMessage defaultMessage='Connect this server to see and pick individual tools, or toggle it on to enable every tool the server exposes once a user connects.'/>
                                        </EmptyToolsNotice>
                                    )}
