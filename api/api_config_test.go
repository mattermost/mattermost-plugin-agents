...
func TestSaveAndGetConfigRoundTrip(t *testing.T) {
	...
	assert.Equal(t, config.SecretPlaceholder, loadedCfg.Services[0].APIKey, "the read response carries the placeholder, not the stored credential")
	assert.Equal(t, "sk-test", store.cfg.Services[0].APIKey, "the credential stays in the store")
	...
	assert.Equal(t, map[string]string{"X-Trace": config.SecretPlaceholder}, loadedCfg.MCP.Servers[0].Headers)
	assert.Equal(t, map[string]string{"Authorization": config.SecretPlaceholder}, loadedCfg.MCP.Servers[0].ServiceAccountHeaders)
	assert.Equal(t, map[string]string{"X-Trace": "on"}, store.cfg.MCP.Servers[0].Headers)
	assert.Equal(t, map[string]string{"Authorization": "Bearer service-pat"}, store.cfg.MCP.Servers[0].ServiceAccountHeaders)
