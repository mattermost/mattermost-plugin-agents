...
type JobStatus struct {
	// JobID uniquely identifies a single run. It is generated by StartReindexJob /
	// StartCatchUpJob and used to scope cancel checks and CAS-gated transitions
	// so that a stale KV read for a previous run cannot interfere with the
	// current run.
...
func (s *Indexer) runReindexJob(jobStatus *JobStatus, clearIndex bool) { //nolint:gocognit
	...
	for {
		// Check if the job was canceled. The check is scoped to this run's
		// JobID so that a stale replica read for a previous run cannot make
		// the worker exit. The transition cancel_requested -> canceled is
		// CAS-gated so concurrent observers do not log the cancel twice and
		// so a superseded run cannot clobber a freshly-started successor.
