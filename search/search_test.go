...
func TestRunSearch(t *testing.T) {
	...
	t.Run("successful RunSearch returns post info", func(t *testing.T) {
		mockEmbedding := mocks.NewMockEmbeddingSearch(t)
		mockClient := mmapimocks.NewMockClient(t)
		searchComplete := make(chan struct{})
		...
		// Second DM is for response post created in the async goroutine.
		mockClient.On("DM", "bot1", "user1", mock.Anything).Return(nil).Once()

		// Empty results keep the async path deterministic for this test.
		...
		}, time.Second, 5*time.Millisecond, "expected async search goroutine to finish within the test")
	})
}
