...
func (p *Plugin) OnActivate() error {
	...
	// Check if migration is needed and trigger save if so
	cfg := p.API.GetConfig()
	if cfg != nil && cfg.PluginSettings.Plugins != nil {
		if settings, ok := cfg.PluginSettings.Plugins["mattermost-ai"]; ok {
			_, changed, err := MigratePluginConfig(settings)
			if err != nil {
				return fmt.Errorf("failed to check for migrations: %w", err)
			}
			if changed {
				pluginAPI.Log.Info("Migration needed, triggering save to apply migrations")
				if err := p.API.SavePluginConfig(settings); err != nil {
					return fmt.Errorf("failed to trigger migration save: %w", err)
				}
			}
		}
	}
