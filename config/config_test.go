...
func TestContainerGetServices(t *testing.T) {
	tests := []struct {
		...
		{
			// Update(nil) stores a zero-value config (Container never holds
			// nil), whose empty Services slice clones to nil.
			name:        "nil configuration yields a nil services snapshot",
			cfg:         nil,
			wantNilCopy: true,
		},
		{
