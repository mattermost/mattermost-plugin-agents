...
type Config struct {
	...
	ChannelJoinSummary              ChannelJoinSummaryConfig         `json:"channelJoinSummary"`
...
// ChannelJoinSummaryConfig controls the ephemeral "catch-up" summary that is
// shown only to a user when they join a channel. The summary is generated by an
// LLM from the channel's recent activity and posted ephemerally (visible to the
// joining user alone, not persisted). Off by default; enabling it runs one LLM
// completion per qualifying join.
type ChannelJoinSummaryConfig struct {
	// Enabled turns the feature on. When false, join events are ignored.
	Enabled bool `json:"enabled"`

	// Bot is the agent username whose LLM generates the summary. Empty falls
	// back to the first configured bot.
	Bot string `json:"bot"`

	// LookbackDays bounds how far back the recent-activity window reaches.
	// Values <= 0 fall back to the built-in default.
	LookbackDays int `json:"lookbackDays"`

	// MinPosts is the minimum number of eligible messages in the window before a
	// summary is generated; quieter channels are skipped to avoid noise. Values
...
func (c *Container) ChannelJoinSummary() ChannelJoinSummaryConfig {
	return c.cfg.Load().ChannelJoinSummary
}
