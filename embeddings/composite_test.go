...
func TestNewCompositeSearch(t *testing.T) {
	tests := []struct {
		name     string
		options  chunking.Options
		validate func(t *testing.T, cs *CompositeSearch)
	}{
		{
			name: "creates CompositeSearch with provided options",
			options: chunking.Options{
				ChunkSize:        500,
				ChunkOverlap:     100,
			...
			validate: func(t *testing.T, cs *CompositeSearch) {
				assert.NotNil(t, cs)
				assert.Equal(t, 500, cs.options.ChunkSize)
				assert.Equal(t, 100, cs.options.ChunkOverlap)
				assert.Equal(t, "paragraphs", cs.options.ChunkingStrategy)
			},
		},
		{
			name: "creates CompositeSearch with zero values",
			options: chunking.Options{
				ChunkSize:    0,
				ChunkOverlap: 0,
			},
			validate: func(t *testing.T, cs *CompositeSearch) {
				assert.NotNil(t, cs)
				assert.Equal(t, 0, cs.options.ChunkSize)
			},
		},
	}
