...
$(shell mkdir -p "$(CURDIR)/build/bin" && cd "$(CURDIR)/build/manifest" && "$(GO)" build -ldflags '-X "main.BuildHashShort=$(BUILD_HASH_SHORT)" -X "main.BuildTagLatest=$(BUILD_TAG_LATEST)" -X "main.BuildTagCurrent=$(BUILD_TAG_CURRENT)"' -o "$(CURDIR)/build/bin/manifest" .)

# Ensure that the deployment tools are compiled. Go's caching makes this quick.
$(shell mkdir -p "$(CURDIR)/build/bin" && cd "$(CURDIR)/build/pluginctl" && "$(GO)" build -o "$(CURDIR)/build/bin/pluginctl" .)

# Extract the plugin id from the manifest.
PLUGIN_ID ?= $(shell "$(CURDIR)/build/bin/manifest" id)
ifeq ($(PLUGIN_ID),)
    $(error "Cannot parse id from $(MANIFEST_FILE)")
endif

# Extract the plugin version from the manifest.
PLUGIN_VERSION ?= $(shell "$(CURDIR)/build/bin/manifest" version)
ifeq ($(PLUGIN_VERSION),)
    $(error "Cannot parse version from $(MANIFEST_FILE)")
endif

# Determine if a server is defined in the manifest.
HAS_SERVER ?= $(shell "$(CURDIR)/build/bin/manifest" has_server)

# Determine if a webapp is defined in the manifest.
HAS_WEBAPP ?= $(shell "$(CURDIR)/build/bin/manifest" has_webapp)
