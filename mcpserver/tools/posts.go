...
func (p *MattermostToolProvider) getPostTools() []MCPTool {
	...
	createDraftDesc := fmt.Sprintf("Save a draft message into a Mattermost channel's message box. A draft is NOT sent — it is saved privately for the user and appears prefilled in their message input box for that channel (or thread). ONLY use this tool when the user explicitly asks to save or create a draft in Mattermost. Do NOT use it for general requests to write or compose a message — phrases like 'draft me a message' usually mean the user wants you to reply with the message text directly, not save a Mattermost draft. There is one draft per channel/thread, so calling this again for the same channel_id (and root_id) overwrites the previous draft. IMPORTANT WORKFLOW: You MUST first call get_channel_info to obtain the channel_id, channel_display_name, and team_display_name. Parameters: channel_id (required), message (required), root_id (optional - to draft a thread reply)%s. Example: {\"channel_id\": \"h5wqm8kxptbztfgzpaxbsqozah\", \"message\": \"Draft for your review\"}", attachmentsParam)
