...
const (
	readChannelDescription = "Read recent posts from a Mattermost channel. Parameters: channel_id (required), per_page (1-200, default 20), page (0+, default 0), since (ISO 8601 timestamp, optional). Posts are returned newest-first across pages; increment page to retrieve more than per_page posts (e.g. page=1 for the next batch). Returns post details including author, content, and timestamps. Example: {\"channel_id\": \"h5wqm8kxptbztfgzpaxbsqozah\", \"per_page\": 50, \"page\": 0, \"since\": \"2024-01-01T00:00:00Z\"}"
