			...
			for _, requestingUserMode := range requestingUserModes {
				for flags := 0; flags < 1<<len(mutators); flags++ {
					context := &llm.Context{
						...
						MCPDynamicToolLoading: toolMode.mcpDynamicToolLoading,
...
func TestStandardPersonalityIncludesDynamicToolWorkflowWhenEnabled(t *testing.T) {
	...
	assert.Contains(t, output, "CRITICAL: For requests that are clear in intent")
}
...
func TestStandardPersonalityOmitsDynamicToolWorkflowWhenFlagOff(t *testing.T) {
	...
	assert.NotContains(t, output, "CRITICAL: For requests that are clear in intent")
}
