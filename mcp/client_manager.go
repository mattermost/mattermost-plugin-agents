...
func (m *ClientManager) GetToolsForUser(userID string) ([]llm.Tool, *Errors) {
	...
	if m.embeddedClient != nil {
		...
		if ensureErr != nil {
			m.log.Debug("Failed to ensure embedded session for user - embedded MCP tools will not be available", "userID", userID)
			...
			if embeddedErr := userClient.ConnectToEmbeddedServerIfAvailable(ensuredSessionID, m.embeddedClient, m.config.EmbeddedServer); embeddedErr != nil {
				m.log.Debug("Failed to connect to embedded server for user - embedded MCP tools will not be available", "userID", userID)
			}
		}
	}
