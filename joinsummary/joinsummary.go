...
const (
	...
	// summaryTimeout bounds the asynchronous generation path (post fetch plus
	// one LLM completion) so a slow or wedged provider can never leak a
...
// to a goroutine so the join is never blocked. The caller-provided ctx is
// propagated (with an added timeout) so the async path stays cancelable rather
// than running unbounded. actor (who performed the add) is currently unused but
// kept to mirror the hook signature and allow future self-join vs added-by-other
// differentiation.
func (s *Service) HandleUserJoinedChannel(ctx context.Context, channelMember *model.ChannelMember, _ *model.User) {
	...
	go func() {
		// Generation is best-effort: recover from any panic so a failure here
		// can never crash the plugin process. Registered first so it runs last
		// and covers the whole goroutine body.
		defer func() {
			if r := recover(); r != nil {
				s.client.LogWarn("channel join summary: recovered from panic during generation", "panic", r, "channel_id", channel.Id)
			}
		}()

		// Derive a bounded context from the caller's so the LLM call cannot hang
		// indefinitely; cancel it once generation completes.
		genCtx, cancel := context.WithTimeout(ctx, summaryTimeout)
		defer cancel()
		s.generateAndSend(genCtx, cfg, userID, channel)
	}()
}
...
func (s *Service) generateAndSend(ctx context.Context, cfg config.ChannelJoinSummaryConfig, userID string, channel *model.Channel) {
	...
	// The ephemeral post must be attributed to the summarizing agent; without a
	// valid bot user we would post with an empty UserId, so bail instead.
	mmBot := bot.GetMMBot()
	if mmBot == nil {
		s.client.LogWarn("channel join summary: bot has no Mattermost user", "channel_id", channel.Id)
		return
	}
