...
func TestPMBotModelComparison(t *testing.T) {
	...
	case "enhanced":
		t.Logf("Comparing %d models in ENHANCED mode (full PM bot with tools): %v", len(models), models)
	case "both":
	...
	// Create real config with full PM functionality (shared across models for cache efficiency)
	sharedLogger := createTestLogger(t, "shared")
	testConfig := CreatePMBotConfig(sharedLogger)
...
func runModelComparison(
	...
	for scenarioIdx, scenario := range scenarios {
		testName := fmt.Sprintf("pmbot %s comparison %s", mode, scenario.Name)
		t.Run(testName, func(t *testing.T) {
			...
			for trialIdx := range numEvals {
				func(t *evals.EvalT) {
					...
					// Test the scenario with each enhanced model
					...
					for modelIdx, modelName := range models {
						...
						go func(modelIdx int, modelName string) {
							...
							if mode == "baseline" {
								...
								// ============ RUN ENHANCED PM BOT VERSION ============
								t.Logf("TRIAL %d/%d [%s-ENHANCED]: Running enhanced PM bot with tools...", trialNum, numEvals, modelName)

								// Create thread data for PM conversation
								threadData := CreatePMBotThreadData(scenario.Message)
									...
									if *warnFlag {
										...
										if len(args) > 1 {
											if fields, ok := args.Get(1).([]interface{}); ok && len(fields) >= 2 {
												...
												// Look for cache hits/misses and datasource calls
												...
												case strings.Contains(msg, "HTTP protocol REJECTION:"):
													// Show HTTP protocol rejection reasons for debugging
													t.Logf("HTTP REJECTION[%s]: %s", modelName, msg)
												case strings.Contains(msg, "QUALITY DEBUG:"):
													// Only show quality failures and provide more detail
													if strings.Contains(msg, "FAIL") || strings.Contains(msg, "FAILED") {
														t.Logf("QUALITY FAIL[%s]: %s", modelName, msg)
													}
												case strings.Contains(msg, "Mattermost protocol") || strings.Contains(msg, "Mattermost Hub") || strings.Contains(msg, "Hub fallback") || strings.Contains(msg, "Loading Hub mock data") || strings.Contains(msg, "Hub mock data"):
													// Show Mattermost protocol debug messages including fallback behavior
													t.Logf("MATTERMOST DEBUG[%s]: %s", modelName, msg)
												}
											}
										}
									}
								...
								// Create real config with full PM functionality
								modelLogger := createTestLogger(t.T, modelName)
								testConfig := CreatePMBotConfig(modelLogger)
								...
								// Process the enhanced PM request
								textStream, err := conv.ProcessUserRequest(bot, threadData.RequestingUser(), threadData.Channel, threadData.LatestPost())
								require.NoError(t.T, err, "Failed to process enhanced PM request with model %s", modelName)
								require.NotNil(t.T, textStream, "Expected a non-nil text stream for model %s", modelName)

								// Read the enhanced response with proper tool handling and capture tool results
								streamResult, err := ProcessStreamWithTools(t.T, textStream, threadData.LatestPost(), contextBuilder, bot, threadData, conv)
								require.NoError(t.T, err, "Failed to read enhanced response from text stream for model %s", modelName)
								require.NotNil(t.T, streamResult, "Expected a non-nil stream result for model %s", modelName)
								response = streamResult.Response
								assert.NotEmpty(t.T, response, "Expected a non-empty enhanced PM response for model %s", modelName)

								t.Logf("TRIAL %d/%d [%s-ENHANCED]: Response received (%d chars, %d tool results)", trialNum, numEvals, modelName, len(response), len(streamResult.ToolResults))
							}
							...
							if len(evalErrors) > 0 {
								...
								return // Skip if there were evaluation errors
							}
...
// getModelsForComparison returns models for comparison
