...
The startup update script keeps deps warm: it sets `go env -w GOTOOLCHAIN=go1.26.4`, runs `make install-go-tools`, and `npm ci` for `webapp/` and `e2e/`. Standard commands are in the `## Commands` section above; the caveats below are the non-obvious parts of this environment.
...
- **Fast end-to-end smoke without the full slow suite or any API keys:** e2e needs Playwright browsers once (`cd e2e && npx playwright install chromium`, cached in the snapshot). Then `cd e2e && npx playwright test --config=playwright.aimock-smoke.config.ts` boots a real Mattermost + pgvector + a deterministic mock-LLM sidecar (aimock), deploys the freshly built `dist/` bundle, and asserts an agent replies in the RHS. The bundle in `dist/` must be exactly one `.tar.gz` (the e2e helper errors otherwise), so run `make clean` if stale bundles accumulate.
