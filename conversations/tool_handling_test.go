...
func (m *capturingLanguageModel) ChatCompletion(_ context.Context, request llm.CompletionRequest, opts ...llm.LanguageModelOption) (*llm.TextStreamResult, error) {
...
func (m *capturingLanguageModel) ChatCompletionNoStream(_ context.Context, _ llm.CompletionRequest, _ ...llm.LanguageModelOption) (string, error) {
	return "", nil
}
...
func TestHandleToolCallPreservesResolvedToolCallsWhenApprovingPendingSubset(t *testing.T) {
	...
	err = conversationService.HandleToolCall(context.Background(), requesterID, post, channel, []string{"tool-2"})
	require.NoError(t, err)
...
func TestHandleToolResultContinuesWhenToolCallErrors(t *testing.T) {
	...
	err = conversationService.HandleToolResult(context.Background(), requesterID, post, channel, []string{"tool-1"})
	require.NoError(t, err)
