...
func (c *Conversations) handleMentions(ctx stdcontext.Context, bot *bots.Bot, post *model.Post, postingUser *model.User, channel *model.Channel) error {
	...
	return c.respondToPost(ctx, bot, postingUser, channel, responsePost, post.Id, func() (*llm.TextStreamResult, error) {
		stream, err := c.ProcessUserRequest(ctx, bot, postingUser, channel, post, allowToolsInChannel, channelToolsAutoRunEverywhereOnly)
		if err != nil {
...
func (c *Conversations) handleDMs(ctx stdcontext.Context, bot *bots.Bot, channel *model.Channel, postingUser *model.User, post *model.Post) error {
	...
	return c.respondToPost(ctx, bot, postingUser, channel, responsePost, post.Id, func() (*llm.TextStreamResult, error) {
		stream, err := c.ProcessUserRequest(ctx, bot, postingUser, channel, post, false, false)
		if err != nil {
			return nil, fmt.Errorf("unable to process bot mention: %w", err)
		}
		return stream, nil
...
func (c *Conversations) respondToPost(
	...
	if err := c.streamResponseToExistingPost(ctx, stream, responsePost, postingUser, channel); err != nil {
		c.failResponsePlaceholder(responsePost, postingUser.Locale)
...
func (c *Conversations) streamResponseToExistingPost(ctx stdcontext.Context, stream *llm.TextStreamResult, post *model.Post, postingUser *model.User, channel *model.Channel) error {
