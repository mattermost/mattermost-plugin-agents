...
func (c *Conversations) HandleRegenerate(ctx stdcontext.Context, userID string, post *model.Post, channel *model.Channel) error {
	...
	default:
		...
		result, processErr = c.ProcessUserRequestWithContext(ctx, bot, user, channel, respondingToPost, contextWithCallback, allowToolsInChannel, channelToolsAutoRunEverywhereOnly)
		if processErr != nil {
