...
func TestResolveApprovedToolUseBlockUsesPersistedMetadata(t *testing.T) {
	...
	result, err := resolveApprovedToolUseBlock(&llm.Context{Tools: store}, conversation.ContentBlock{
...
func TestResolveApprovedToolUseBlockRejectsServerOriginMismatch(t *testing.T) {
	...
	_, err := resolveApprovedToolUseBlock(&llm.Context{Tools: store}, conversation.ContentBlock{
		Name:         "jira__get_issue",
		ServerOrigin: "https://jira.example.com",
...
func TestResolveApprovedToolUseBlockNoLongerAvailable(t *testing.T) {
	_, err := resolveApprovedToolUseBlock(&llm.Context{Tools: llm.NewNoTools()}, conversation.ContentBlock{
		Name:  "jira__get_issue",
		Input: json.RawMessage(`{}`),
	})
